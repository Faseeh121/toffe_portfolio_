...
export default function BlogWebsite({ onBack }: PageProps) {
  const screenshots = [
    { src: "/images/blog_features.png", label: "Homepage — Feed, Stats & Featured Posts" },
    { src: "/images/blog_login.png", label: "Login — Secure Authentication Flow" },
    { src: "/images/Blog_register.png", label: "Register — New User Onboarding" },
    { src: "/images/Blog_chat_assistant.png", label: "Post Detail — AI Chat Assistant Integration" },
  ];

  const features = [
    { icon: "📝", title: "Rich Post Editor", description: "Create and publish posts with rich text formatting, inline images, and cloud-hosted media uploads" },
    { icon: "🔐", title: "Secure Authentication", description: "JWT-based auth with protected routes, profile management, and session persistence" },
    { icon: "📒", title: "Personal Notes", description: "Readers can attach private notes to any post for future reference and study" },
    { icon: "🤖", title: "AI Chat Assistant", description: "Integrated chat assistant for interactive Q&A about blog content and topics" },
    { icon: "📱", title: "Responsive Design", description: "Fully responsive layouts optimized for desktop, tablet, and mobile screen sizes" },
    { icon: "☁️", title: "Cloud Image Hosting", description: "Automatic image uploads to cloud storage with optimized delivery and CDN caching" },
  ];

  const techStack = ["MongoDB", "Express.js", "React", "Node.js", "JWT Auth", "Cloud Storage"];
  ...
  return (
    <div style={s.page as any}>
      <div style={s.grain} />
      <motion.div style={{ ...s.orbA, background: "radial-gradient(circle, rgba(139,92,246,0.12) 0%, transparent 70%)" }} animate={{ y: [0, -40, 0], scale: [1, 1.15, 1] }} transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div style={{ ...s.orbB, background: "radial-gradient(circle, rgba(167,139,250,0.10) 0%, transparent 70%)" }} animate={{ y: [0, 50, 0], scale: [1, 1.2, 1] }} transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }} />
      ...
      <header ref={heroRef} style={s.hero}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={heroInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} style={s.heroInner}>
          <motion.div initial={{ opacity: 0, scale: 0 }} animate={heroInView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.2, type: "spring", stiffness: 200 }} style={{ ...s.heroBadge, background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.25)", color: accentColor }}>
            📝 Blog Platform
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={heroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3, duration: 0.8 }} style={s.heroTitle}>
            Clean{" "}
            <span style={{ background: "linear-gradient(135deg, #8b5cf6 0%, #c4b5fd 50%, #a78bfa 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
              Blog
            </span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={heroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5, duration: 0.6 }} style={s.heroSub}>
            A MERN-based platform to publish rich articles with secure authentication, cloud image uploads, personal notes, AI assistant, and a polished responsive UI.
          </motion.p>
        </motion.div>
      </header>
      ...
      <section style={s.section}>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} style={{ marginTop: 48 }}>
          ...
          <div style={s.techRow}>
            {techStack.map((t) => (
              <motion.span key={t} whileHover={{ scale: 1.08, borderColor: accentColor }} style={{ ...s.techTag, borderColor: `${accentColor}40` }}>{t}</motion.span>
            ))}
          </div>
        </motion.div>
      </section>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} style={s.footerCta}>
        {onBack && (
          <motion.button whileHover={{ scale: 1.05, boxShadow: "0 8px 30px rgba(139, 92, 246, 0.4)" }} whileTap={{ scale: 0.95 }} onClick={onBack} style={{ ...s.footerCtaBtn, background: "linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%)" }}>
            ← Return to Websites
          </motion.button>
        )}
      </motion.div>
...
const s: Record<string, CSSProperties> = {
  ...
  hero: { position: "relative", minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "120px clamp(24px, 6vw, 80px) 40px", zIndex: 1 },
  ...
  heroSub: { fontSize: "clamp(1.05rem, 2vw, 1.3rem)", color: "rgba(255,255,255,0.55)", lineHeight: 1.7, maxWidth: 680, margin: "0 auto" },
  ...
  techTag: { padding: "7px 16px", borderRadius: 10, border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.08)", fontSize: "0.85rem", fontWeight: 600, color: "#e0e0ff", transition: "all 0.25s", cursor: "default" },
  footerCta: { position: "relative", zIndex: 1, textAlign: "center", padding: "40px clamp(24px, 6vw, 80px) 80px" },
  footerCtaBtn: { padding: "16px 40px", fontSize: "1rem", fontWeight: 700, color: "#fff", border: "none", borderRadius: 14, cursor: "pointer", transition: "all 0.3s" },
};
